...
namespace robot_planning
{
    ...
    IKSolver::IKSolver(const IKSolverConfig& config) : config_(config)
    {
        ...
        if (config_joint_limits["floating_joint"])
        {
            ...
            {
                floating_min_.head<3>() = Eigen::Vector3d(config_joint_limits["floating_joint"]["position"]["min"][0].as<double>(), config_joint_limits["floating_joint"]["position"]["min"][1].as<double>(), config_joint_limits["floating_joint"]["position"]["min"][2].as<double>());
                floating_max_.head<3>() = Eigen::Vector3d(config_joint_limits["floating_joint"]["position"]["max"][0].as<double>(), config_joint_limits["floating_joint"]["position"]["max"][1].as<double>(), config_joint_limits["floating_joint"]["position"]["max"][2].as<double>());
            }

            // 读取旋转信息
            if (config_joint_limits["floating_joint"]["rotation"])
            {
                Eigen::Vector3d euler_min = Eigen::Vector3d(DEG2RAD(config_joint_limits["floating_joint"]["rotation"]["euler_min"][0].as<double>()), DEG2RAD(config_joint_limits["floating_joint"]["rotation"]["euler_min"][1].as<double>()),
                                                            DEG2RAD(config_joint_limits["floating_joint"]["rotation"]["euler_min"][2].as<double>()));
                Eigen::Vector3d euler_max = Eigen::Vector3d(DEG2RAD(config_joint_limits["floating_joint"]["rotation"]["euler_max"][0].as<double>()), DEG2RAD(config_joint_limits["floating_joint"]["rotation"]["euler_max"][1].as<double>()),
                                                            DEG2RAD(config_joint_limits["floating_joint"]["rotation"]["euler_max"][2].as<double>()));

                Eigen::Quaterniond quat_min = Eigen::AngleAxisd(euler_min[0], Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(euler_min[1], Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(euler_min[2], Eigen::Vector3d::UnitZ());
                Eigen::Quaterniond quat_max = Eigen::AngleAxisd(euler_max[0], Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(euler_max[1], Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(euler_max[2], Eigen::Vector3d::UnitZ());
    ...
    IKSolverResult IKSolver::solveIK(const Eigen::Vector3d& target_pos_R, const Eigen::Matrix3d& target_ori_R, const Eigen::Vector3d& target_pos_L, const Eigen::Matrix3d& target_ori_L)
    {
        ...
        Eigen::Quaterniond quat_init(qIk[6], qIk[3], qIk[4], qIk[5]);  // w, x, y, z
        quat_init.normalize();
        qIk[3] = quat_init.x();
        qIk[4] = quat_init.y();
        qIk[5] = quat_init.z();
        ...
        for (; iter_count < config_.max_iterations; ++iter_count)
        {
            ...
            JJt_L.diagonal().array() += config_.damp;
            JJt_R.diagonal().array() += config_.damp;
            ...
            // ✅ 2. 立即归一化 quaternion（顺序：w, x, y, z）
            Eigen::Quaterniond quat(qIk[6], qIk[3], qIk[4], qIk[5]);
            quat.normalize();
            qIk[3] = quat.x();
            qIk[4] = quat.y();
            qIk[5] = quat.z();
